...
public class StudentDataInitializer implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    String formattedPath =
        csvPath.endsWith("/") || csvPath.endsWith("\\") ? csvPath : csvPath + File.separator;
    ...
    try (BufferedReader br =
      ...
      while ((line = br.readLine()) != null) {
        ...
        if (status.equals("재학생")) {
          hash =
              IdentityHasher.hashIdentity(
                  data[0].trim(), data[1].trim(), status); // 재학생: data[0]=학번, data[1]=이름
        } else if (status.equals("휴학생")) {
          if (data.length >= 3) {
            hash =
                IdentityHasher.hashIdentity(
                    data[1].trim(), data[2].trim(), status); // 휴학생: data[1]=학번, data[2]=이름
          }
        }
