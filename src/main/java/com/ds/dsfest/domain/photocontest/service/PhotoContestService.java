...
public class PhotoContestService {
    ...
    public PhotoListResDto getPhotoList() {
        List<PhotoListResDto.PhotoSummaryDto> allPhotos = photoEntryRepository.findAllByOrderByIdAsc()
            .stream()
    ...
    public void votePhotos(PhotoVoteReqDto reqDto) {
        ...
        /** 선택한 사진 조회 및 검증 (단일 건) */
        PhotoEntry selectedPhoto = photoEntryRepository.findById(reqDto.photoEntryId())
            .orElseThrow(() -> new CustomException(GlobalErrorCode.NOT_FOUND));
