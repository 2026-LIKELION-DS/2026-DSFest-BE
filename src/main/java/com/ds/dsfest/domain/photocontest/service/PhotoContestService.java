...
public class PhotoContestService {
  ...
  public PhotoListResDto getPhotoList() {
    ...
    List<PhotoListResDto.PhotoSummaryDto> youthPhotos =
        allPhotos.stream()
            .filter(photo -> photo.getTheme() == PhotoTheme.YOUTH)
            .map(PhotoListResDto.PhotoSummaryDto::from)
            .toList();

    List<PhotoListResDto.PhotoSummaryDto> festivalPhotos =
        allPhotos.stream()
            .filter(photo -> photo.getTheme() == PhotoTheme.FESTIVAL)
            .map(PhotoListResDto.PhotoSummaryDto::from)
            .toList();

    List<PhotoListResDto.PhotoSummaryDto> dressCodePhotos =
        allPhotos.stream()
  ...
  public void votePhotos(PhotoVoteReqDto reqDto) {
    ...
    List<PhotoVote> votes =
        selectedPhotos.stream().map(photo -> new PhotoVote(photo, finalVoterKey)).toList();
  ...
  public Map<String, List<PhotoRankResDto>> getVoteResults() {
    ...
    return allRanks.stream().collect(Collectors.groupingBy(PhotoRankResDto::theme));
  }
}
